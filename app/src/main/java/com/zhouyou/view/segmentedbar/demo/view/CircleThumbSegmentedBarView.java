...
public class CircleThumbSegmentedBarView extends SegmentedBarView {
    ...
    protected void drawCustomThumb(Canvas canvas, int thumbW, int thumbH, float centerX, float centerY, int color, Paint paint) {
        ...
        int r = Math.max(thumbH, thumbW) / 2;//计算圆的半径
        int bgShadowColor = Color.argb(80, 0, 0, 0);
        mPaint.setShadowLayer(10, 0, 0, bgShadowColor);//加阴影效果
        //mPaint.setMaskFilter(new BlurMaskFilter(50, BlurMaskFilter.Blur.SOLID));//另外一种周围发光效果setMaskFilter
        canvas.drawCircle(centerX, centerY, r-10, mPaint);//大圆
        mPaint.clearShadowLayer();//清空掉阴影
        //mPaint.setMaskFilter(null);
        mPaint.setColor(color);
        canvas.drawCircle(centerX, centerY, r-18, mPaint);//小圆
    }
}
